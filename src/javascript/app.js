...
Matter.Events.on(engine, "afterUpdate", function() {
  ...
  const limit = width / 1.875 + chopstickLength / 3;
  ...
  Body.translate(chopstick1, {
    x: (position - chopstick1.position.x - offset1.x) * 0.35,
    y: (mouse.position.y - chopstick1.position.y + offset1.y) * 0.1
  });
  Body.translate(chopstick2, {
    x: (position - chopstick2.position.x - offset2.x) * 0.2,
...
window.addEventListener("mousedown", function() {
  ...
  Body.setAngle(chopstick1, -Math.PI * 0.080);
  Body.setAngle(chopstick2, -Math.PI * 0.050);
});
